...
const getUpdatedData = async(req,res)=>{
    try{
        const Result = await RegistionDataNew.findOne({_id:req.params.id}).populate({path:"BookMark.users",populate:{path:"users",model:"RegistarionDataNew"}})
        res.json({
          Result
        }) 
      }
...
module.exports = { Registation,login,patchUserData,getUpdatedData,getAllUsers,bookMarkPost}